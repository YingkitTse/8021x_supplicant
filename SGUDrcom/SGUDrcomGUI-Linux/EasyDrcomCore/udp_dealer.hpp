...
#if defined(__APPLE__) || defined(__MACH__) || defined(LINUX) || defined(linux) || defined(__linux__)
#include <fcntl.h>
#endif
...
{
public:
	udp_dealer(std::string gateway_ip, uint32_t gateway_port, std::string local_ip) : sock(-1)
    {
...
#define U31_PERFORM(step) U31_PERFORM_EX(step, pkt_data, handler_success, handler_error)

#define U31_PERFORM_EX(step, data, success, error)                                                      \
{                                                                                                       \
    int ret;                                                                                            \
    try {                                                                                               \
        if ((ret = udp.post(data, success, error)) < 0)                                                 \
        {                                                                                               \
...
#define U62_PERFORM(step) U62_PERFORM_EX(step, pkt_data, handler_success, handler_error)

#define U62_PERFORM_EX(step, data, success, error)                                                      \
{                                                                                                       \
    int ret;                                                                                            \
    try {                                                                                               \
        if ((ret = udp.post(data, success, error)) < 0)                                                 \
