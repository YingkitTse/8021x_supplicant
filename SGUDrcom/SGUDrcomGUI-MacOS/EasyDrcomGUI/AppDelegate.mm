...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
    ...
    [self.lblBuild setStringValue: [NSString stringWithFormat:@"Modified by YingKit, build on %s %s.", __DATE__, __TIME__]];
    
    SYS_LOG_INFO("EasyDrcomGUI For OSX, v" << [version UTF8String] << std::endl);
    SYS_LOG_INFO("Modified by YingKit, build on " __DATE__ " " __TIME__ << std::endl);
...
- (void) keepAliveForStudentDistrictWithDealer: (DrcomDealerU62*) dealer
{
    ...
    try {
        ...
        {
            [NSThread sleepForTimeInterval:30.0f]; // 30s for alive
